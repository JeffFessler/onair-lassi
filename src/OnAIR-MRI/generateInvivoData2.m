function [Y, Xtrue, X0] = generateInvivoData2(nLines,SNR,data)
% Syntax:   [Y, Xtrue, X0] = generateInvivoData2(nLines,SNR,inpath);
%           [Y, Xtrue, X0] = generateInvivoData2(nLines,SNR,data);
...
%{
% Method #2: Data filled back-propagation
Yfill = data_share_fill(Y(mask),mask);
X0 = sqrt(data.nx * data.ny) * ifft2(Yfill);
%}
